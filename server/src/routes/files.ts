...
function formatFile(file: TripFile) {
  return {
    ...file,
    url: file.filename?.startsWith("files/")
...
router.get("/", authenticate, (req: Request, res: Response) => {
  ...
  const where = showTrash
    ? "f.trip_id = ? AND f.deleted_at IS NOT NULL"
    : "f.trip_id = ? AND f.deleted_at IS NULL";
  const files = db
    .prepare(
  ...
  res.json({
    files: files.map((f) => {
      ...
      return {
        ...formatFile(f),
        linked_reservation_ids: fileLinks
          .filter((l) => l.reservation_id)
          .map((l) => l.reservation_id),
        linked_place_ids: fileLinks
          .filter((l) => l.place_id)
          .map((l) => l.place_id),
      };
    }),
...
router.post(
  ...
  `,
    ...
    const file = db
      .prepare(`${FILE_SELECT} WHERE f.id = ?`)
...
router.put("/:id", authenticate, (req: Request, res: Response) => {
  ...
  const file = db
    .prepare("SELECT * FROM trip_files WHERE id = ? AND trip_id = ?")
...
router.patch("/:id/star", authenticate, (req: Request, res: Response) => {
  ...
  db.prepare("UPDATE trip_files SET starred = ? WHERE id = ?").run(
    newStarred,
    id,
  );
...
router.delete("/:id", authenticate, (req: Request, res: Response) => {
  ...
  db.prepare(
    "UPDATE trip_files SET deleted_at = CURRENT_TIMESTAMP WHERE id = ?",
  ).run(id);
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    await deleteFile(`files/${file.filename}`);
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    db.prepare(
      "DELETE FROM trip_files WHERE trip_id = ? AND deleted_at IS NOT NULL",
    ).run(tripId);
...
router.post("/:id/link", authenticate, (req: Request, res: Response) => {
  ...
  try {
    ...
    ).run(id, reservation_id || null, assignment_id || null, place_id || null);
  } catch {}

  const links = db
    .prepare("SELECT * FROM file_links WHERE file_id = ?")
    .all(id);
...
router.delete(
  ...
  (req: Request, res: Response) => {
    ...
    db.prepare("DELETE FROM file_links WHERE id = ? AND file_id = ?").run(
      linkId,
      id,
    );
    res.json({ success: true });
