...
router.put("/notes/:id", authenticate, (req: Request, res: Response) => {
  ...
  const existing = db
    .prepare("SELECT * FROM collab_notes WHERE id = ? AND trip_id = ?")
    .get(id, tripId);
...
router.post(
  ...
  (req: Request, res: Response) => {
    ...
    const file = db
      .prepare("SELECT * FROM trip_files WHERE id = ?")
      .get(result.lastInsertRowid) as TripFile;
    res
      .status(201)
      .json({ file: { ...file, url: `/uploads/${file.filename}` } });
    broadcast(
      Number(tripId),
      "collab:note:updated",
      {
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    const file = db
      ...
      .get(fileId, id) as TripFile | undefined;
...
function getPollWithVotes(pollId: number | bigint | string) {
  ...
  const formattedOptions = options.map(
    (label: string | { label: string }, idx: number) => ({
...
router.post("/polls/:id/vote", authenticate, (req: Request, res: Response) => {
  ...
  const poll = db
    ...
    .get(id, tripId) as CollabPoll | undefined;
  ...
  if (existingVote) {
    db.prepare("DELETE FROM collab_poll_votes WHERE id = ?").run(
      existingVote.id,
    );
  } else {
    if (!poll.multiple) {
      db.prepare(
        "DELETE FROM collab_poll_votes WHERE poll_id = ? AND user_id = ?",
      ).run(id, authReq.user.id);
    }
    db.prepare(
      "INSERT INTO collab_poll_votes (poll_id, user_id, option_index) VALUES (?, ?, ?)",
    ).run(id, authReq.user.id, option_index);
  }
...
router.get("/messages", authenticate, (req: Request, res: Response) => {
  ...
  res.json({
    messages: messages.map((m) =>
      formatMessage(m, groupReactions(reactionsByMsg[m.id] || [])),
    ),
  });
});
...
router.post(
  "/messages",
  authenticate,
  validateStringLengths({ text: 5000 }),
  (req: Request, res: Response) => {
    const authReq = req as AuthRequest;
    const { tripId } = req.params;
    ...
    if (reply_to) {
      ...
      if (!replyMsg)
        return res
          .status(400)
          .json({ error: "Reply target message not found" });
    }
...
router.post(
  ...
  (req: Request, res: Response) => {
    ...
    const existing = db
      ...
      .get(id, authReq.user.id, emoji) as { id: number } | undefined;
    if (existing) {
      db.prepare("DELETE FROM collab_message_reactions WHERE id = ?").run(
        existing.id,
      );
    } else {
      db.prepare(
        "INSERT INTO collab_message_reactions (message_id, user_id, emoji) VALUES (?, ?, ?)",
      ).run(id, authReq.user.id, emoji);
    }
...
router.delete("/messages/:id", authenticate, (req: Request, res: Response) => {
  ...
  const message = db
    ...
    .get(id, tripId) as CollabMessage | undefined;
  ...
  broadcast(
    ...
    {
      messageId: Number(id),
      username: message.username || authReq.user.username,
    },
    req.headers["x-socket-id"] as string,
  );
});
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (
        /^(127\.|10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.|0\.|169\.254\.|::1|::ffff:(127\.|10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.))/.test(
          ip,
        )
      ) {
        return res
      ...
      nodeFetch(url, {
        ...
        headers: {
          "User-Agent":
            "Mozilla/5.0 (compatible; NOMAD/1.0; +https://github.com/mauriceboe/NOMAD)",
        },
      })
        ...
        .then((html: string) => {
          ...
          res.json({
            ...
            description:
              get("description") || (descMeta ? descMeta[1].trim() : null),
