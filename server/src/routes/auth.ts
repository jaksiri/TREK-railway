...
import { deleteFile, tempDir } from "../services/s3";
...
function hashBackupCode(input: string): string {
  return crypto
    .createHash("sha256")
    .update(normalizeBackupCode(input))
    .digest("hex");
}
...
const avatarUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (
      ...
      !ALLOWED_AVATAR_EXTS.includes(ext)
    ) {
      return cb(
...
function isOidcOnlyMode(): boolean {
  ...
  const enabled =
    process.env.OIDC_ONLY === "true" || get("oidc_only") === "true";
...
router.get("/app-config", (_req: Request, res: Response) => {
  ...
  const setting = db
    .prepare("SELECT value FROM app_settings WHERE key = 'allow_registration'")
    .get() as { value: string } | undefined;
  const allowRegistration =
    userCount === 0 || (setting?.value ?? "true") === "true";
  ...
  const oidcDisplayName =
    ...
    (
      db
        .prepare(
          "SELECT value FROM app_settings WHERE key = 'oidc_display_name'",
        )
        .get() as { value: string } | undefined
    )?.value ||
    null;
  const oidcConfigured = !!(
    (process.env.OIDC_ISSUER ||
      (
        db
          .prepare("SELECT value FROM app_settings WHERE key = 'oidc_issuer'")
          .get() as { value: string } | undefined
      )?.value) &&
    (process.env.OIDC_CLIENT_ID ||
      (
        db
          .prepare(
            "SELECT value FROM app_settings WHERE key = 'oidc_client_id'",
          )
          .get() as { value: string } | undefined
      )?.value)
  );
  const oidcOnlySetting =
    process.env.OIDC_ONLY ||
    (
      db
        .prepare("SELECT value FROM app_settings WHERE key = 'oidc_only'")
        .get() as { value: string } | undefined
    )?.value;
  const oidcOnlyMode = oidcConfigured && oidcOnlySetting === "true";
  const requireMfaRow = db
...
router.post("/demo-login", (_req: Request, res: Response) => {
  ...
  const user = db
    ...
    .get("demo@trek.app") as User | undefined;
  ...
  if (userCount > 0 && !validInvite) {
    if (isOidcOnlyMode()) {
      return res
        ...
        .json({
          error:
            "Password authentication is disabled. Please sign in with SSO.",
        });
    }
    const setting = db
      .prepare(
...
router.post("/login", authLimiter, (req: Request, res: Response) => {
  if (isOidcOnlyMode()) {
    return res
      .status(403)
      .json({
        error: "Password authentication is disabled. Please sign in with SSO.",
      });
  }
...
router.put(
  "/me/password",
  authenticate,
  rateLimiter(5, RATE_LIMIT_WINDOW),
  (req: Request, res: Response) => {
    const authReq = req as AuthRequest;
    if (isOidcOnlyMode()) {
      return res
        .status(403)
        .json({ error: "Password authentication is disabled." });
    }
    if (
      process.env.DEMO_MODE === "true" &&
      authReq.user.email === "demo@trek.app"
    ) {
      return res
        .status(403)
        .json({ error: "Password change is disabled in demo mode." });
    }
    ...
    const user = db
      ...
      .get(authReq.user.id) as { password_hash: string } | undefined;
...
router.delete("/me", authenticate, (req: Request, res: Response) => {
  ...
  if (
    ...
    return res
      .status(403)
      .json({ error: "Account deletion is disabled in demo mode." });
  }
...
router.put("/me/api-keys", authenticate, (req: Request, res: Response) => {
  ...
  const u = updated
    ? {
        ...updated,
        mfa_enabled: !!(
          updated.mfa_enabled === 1 || updated.mfa_enabled === true
        ),
      }
    : undefined;
  res.json({
...
router.put("/me/settings", authenticate, (req: Request, res: Response) => {
  ...
  if (username !== undefined) {
    ...
    if (!trimmed || trimmed.length < 2 || trimmed.length > 50) {
      return res
        .status(400)
        .json({ error: "Username must be between 2 and 50 characters" });
    }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const current = db
      ...
      .get(authReq.user.id) as { avatar: string | null } | undefined;
    if (current && current.avatar) {
      await deleteFile(`avatars/${current.avatar}`);
    }

    const filename = req.file.filename;
    db.prepare(
      "UPDATE users SET avatar = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
    ).run(filename, authReq.user.id);
...
router.get("/app-settings", authenticate, (req: Request, res: Response) => {
  ...
  const user = db
    ...
    .get(authReq.user.id) as { role: string } | undefined;
  if (user?.role !== "admin")
    return res.status(403).json({ error: "Admin access required" });
...
router.put("/app-settings", authenticate, (req: Request, res: Response) => {
  ...
  const { allow_registration, allowed_file_types, require_mfa } =
    req.body as Record<string, unknown>;

  if (require_mfa === true || require_mfa === "true") {
    const adminMfa = db
      .prepare("SELECT mfa_enabled FROM users WHERE id = ?")
      .get(authReq.user.id) as { mfa_enabled: number } | undefined;
    if (!(adminMfa?.mfa_enabled === 1)) {
      return res.status(400).json({
        error:
          "Enable two-factor authentication on your own account before requiring it for all users.",
      });
    }
  }
  ...
  places.forEach((p) => {
    ...
    if (p.address) {
      ...
      const cityPart = parts.find(
        (s) =>
          !KNOWN_COUNTRIES.has(s) && /^[A-Za-z\u00C0-\u00FF\s-]{2,}$/.test(s),
      );
...
router.post("/mfa/verify-login", authLimiter, (req: Request, res: Response) => {
  ...
  if (!mfa_token || !code) {
    return res
      .status(400)
      .json({ error: "Verification token and code are required" });
  }
  try {
    const decoded = jwt.verify(mfa_token, JWT_SECRET) as {
      id: number;
      purpose?: string;
    };
...
router.post("/mfa/setup", authenticate, (req: Request, res: Response) => {
  ...
  if (
    process.env.DEMO_MODE === "true" &&
    authReq.user.email === "demo@nomad.app"
  ) {
    return res
      .status(403)
      .json({ error: "MFA is not available in demo mode." });
  }
  const row = db
    .prepare("SELECT mfa_enabled FROM users WHERE id = ?")
    .get(authReq.user.id) as { mfa_enabled: number } | undefined;
  ...
  try {
    ...
    mfaSetupPending.set(authReq.user.id, {
      secret,
      exp: Date.now() + MFA_SETUP_TTL_MS,
    });
...
router.post("/mfa/enable", authenticate, (req: Request, res: Response) => {
  ...
  if (!pending) {
    return res
      .status(400)
      .json({ error: "No MFA setup in progress. Start the setup again." });
  }
  ...
  ).run(enc, JSON.stringify(backupHashes), authReq.user.id);
...
router.post(
  ...
  (req: Request, res: Response) => {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "MFA cannot be changed in demo mode." });
    }
    const policy = db
      .prepare("SELECT value FROM app_settings WHERE key = 'require_mfa'")
      .get() as { value: string } | undefined;
    if (policy?.value === "true") {
      return res
        ...
        .json({
          error:
            "Two-factor authentication cannot be disabled while it is required for all users.",
        });
    }
    ...
    if (!password || !code) {
      return res
        .status(400)
        .json({ error: "Password and authenticator code are required" });
    }
    const user = db
      .prepare("SELECT * FROM users WHERE id = ?")
      .get(authReq.user.id) as User | undefined;
...
router.post(
  ...
  (req: Request, res: Response) => {
    ...
    if (tokenCount >= 10)
      return res
        .status(400)
        .json({ error: "Maximum of 10 tokens per user reached" });
    ...
    res
      .status(201)
      .json({ token: { ...(token as object), raw_token: rawToken } });
  },
);
...
router.delete(
  ...
  (req: Request, res: Response) => {
    ...
    const { id } = req.params;
    const token = db
      .prepare("SELECT id FROM mcp_tokens WHERE id = ? AND user_id = ?")
