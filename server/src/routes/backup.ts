...
type RestoreAuditInfo = {
  userId: number;
  ip: string | null;
  source: "backup.restore" | "backup.upload_restore";
  label: string;
};
...
function backupRateLimiter(maxAttempts: number, windowMs: number) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      ...
      return res
        .status(429)
        .json({ error: "Too many backup requests. Please try again later." });
    }
...
router.post(
  "/create",
  backupRateLimiter(3, BACKUP_RATE_WINDOW),
...
async function restoreFromZip(
  ...
  try {
    ...
    if (!fs.existsSync(extractedDb)) {
      ...
      return res
        .status(400)
        .json({ error: "Invalid backup: travel.db not found" });
    }
    ...
    try {
      ...
      if (fs.existsSync(extractedUploads)) {
        ...
        for (const filePath of walkDir(extractedUploads)) {
          const key = path
            .relative(extractedUploads, filePath)
            .replace(/\\/g, "/");
...
function parseAutoBackupBody(body: Record<string, unknown>): {
  ...
  const interval =
    ...
    scheduler.VALID_INTERVALS.includes(rawInterval)
      ? rawInterval
      : "daily";
  const keep_days = Math.max(0, parseIntField(body.keep_days, 7));
  const hour = Math.min(23, Math.max(0, parseIntField(body.hour, 2)));
  const day_of_week = Math.min(
    6,
    Math.max(0, parseIntField(body.day_of_week, 0)),
  );
  const day_of_month = Math.min(
    28,
    Math.max(1, parseIntField(body.day_of_month, 1)),
  );
...
router.put("/auto-settings", (req: Request, res: Response) => {
  try {
    const settings = parseAutoBackupBody(
