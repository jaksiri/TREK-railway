...
[backupsDir, tmpDir, tempDir].forEach((dir) => {
...
if (DEBUG) {
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    console.log(
      `[DEBUG][REQ ${requestId}] ${req.method} ${req.originalUrl} ip=${req.ip} query=${JSON.stringify(safeQuery)} body=${JSON.stringify(safeBody)}`,
    );

    res.on("finish", () => {
      const elapsedMs = Date.now() - startedAt;
      console.log(
        `[DEBUG][RES ${requestId}] ${req.method} ${req.originalUrl} status=${res.statusCode} elapsed_ms=${elapsedMs}`,
      );
    });
...
import daysRoutes, {
  accommodationsRouter as accommodationsRoutes,
} from "./routes/days";
...
app.get("/api/addons", addonAuth, (req: Request, res: Response) => {
  const addons = addonDb
    .prepare(
      "SELECT id, name, type, icon, enabled FROM addons WHERE enabled = 1 ORDER BY sort_order",
    )
...
const server = app.listen(PORT, () => {
  ...
  if (
    ...
    console.warn(
      "[SECURITY WARNING] DEMO_MODE is enabled in production! Demo credentials are publicly exposed.",
    );
  }
  scheduler.start();
