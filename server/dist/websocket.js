...
function setupWebSocket(server) {
    ...
    wss.on('connection', (ws, req) => {
        ...
        try {
            ...
            user = database_1.db.prepare('SELECT id, username, email, role, mfa_enabled FROM users WHERE id = ?').get(decoded.id);
            if (!user) {
                nws.close(4001, 'User not found');
                return;
            }
            const requireMfa = database_1.db.prepare("SELECT value FROM app_settings WHERE key = 'require_mfa'").get()?.value === 'true';
