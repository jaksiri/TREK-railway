...
exports.notifyTripMembers = notifyTripMembers;
...
function getUserEmail(userId) {
    return database_1.db.prepare('SELECT email FROM users WHERE id = ?').get(userId)?.email || null;
}
function getUserLanguage(userId) {
    return database_1.db.prepare("SELECT value FROM settings WHERE user_id = ? AND key = 'language'").get(userId)?.value || 'en';
}
function getUserPrefs(userId) {
    const row = database_1.db.prepare('SELECT * FROM notification_preferences WHERE user_id = ?').get(userId);
...
function getEventText(lang, event, params) {
    const texts = EVENT_TEXTS[lang] || EVENT_TEXTS.en;
    return texts[event](params);
}
// ── Email HTML builder ─────────────────────────────────────────────────────
function buildEmailHtml(subject, body, lang) {
...
async function notify(payload) {
    ...
    if (email)
        await sendEmail(email, title, body, payload.userId);
    if (prefs.notify_webhook)
        await sendWebhook({ event: payload.event, title, body, tripName: payload.params.trip });
}
async function notifyTripMembers(tripId, actorUserId, event, params) {
    const trip = database_1.db.prepare('SELECT user_id FROM trips WHERE id = ?').get(tripId);
    if (!trip)
        return;
    const members = database_1.db.prepare('SELECT user_id FROM trip_members WHERE trip_id = ?').all(tripId);
