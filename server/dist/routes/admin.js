...
router.post('/save-demo-baseline', (req, res) => {
    ...
    try {
        ...
        saveBaseline();
        const authReq = req;
        (0, auditLog_1.writeAudit)({ userId: authReq.user.id, action: 'admin.demo_baseline_save', ip: (0, auditLog_1.getClientIp)(req) });
...
router.post('/invites', (req, res) => {
    ...
    const expiresAt = expires_in_days
        ...
        : null;
    const ins = database_1.db.prepare('INSERT INTO invite_tokens (token, max_uses, expires_at, created_by) VALUES (?, ?, ?, ?)').run(token, uses, expiresAt, authReq.user.id);
...
router.delete('/invites/:id', (req, res) => {
    const invite = database_1.db.prepare('SELECT id FROM invite_tokens WHERE id = ?').get(req.params.id);
    if (!invite)
        return res.status(404).json({ error: 'Invite not found' });
    database_1.db.prepare('DELETE FROM invite_tokens WHERE id = ?').run(req.params.id);
    const authReq = req;
    (0, auditLog_1.writeAudit)({
        userId: authReq.user.id,
...
router.delete('/packing-templates/:id', (req, res) => {
    const template = database_1.db.prepare('SELECT * FROM packing_templates WHERE id = ?').get(req.params.id);
    if (!template)
        return res.status(404).json({ error: 'Template not found' });
    database_1.db.prepare('DELETE FROM packing_templates WHERE id = ?').run(req.params.id);
    const authReq = req;
    const t = template;
    (0, auditLog_1.writeAudit)({
        userId: authReq.user.id,
...
router.get('/mcp-tokens', (req, res) => {
    const tokens = database_1.db.prepare(`
...
router.delete('/mcp-tokens/:id', (req, res) => {
    const token = database_1.db.prepare('SELECT id, user_id FROM mcp_tokens WHERE id = ?').get(req.params.id);
    if (!token)
        return res.status(404).json({ error: 'Token not found' });
    database_1.db.prepare('DELETE FROM mcp_tokens WHERE id = ?').run(req.params.id);
