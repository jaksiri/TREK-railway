...
const avatarStorage = multer_1.default.diskStorage({
    destination: (_req, _file, cb) => { if (!fs_1.default.existsSync(s3_1.tempDir))
        fs_1.default.mkdirSync(s3_1.tempDir, { recursive: true }); cb(null, s3_1.tempDir); },
...
router.post('/avatar', auth_1.authenticate, auth_1.demoUploadBlock, avatarUpload.single('avatar'), (0, s3Upload_1.s3Upload)('avatars'), async (req, res) => {
    const authReq = req;
    if (!req.file)
        return res.status(400).json({ error: 'No image uploaded' });
    const current = database_1.db.prepare('SELECT avatar FROM users WHERE id = ?').get(authReq.user.id);
    if (current && current.avatar) {
        await (0, s3_1.deleteFile)(`avatars/${current.avatar}`);
    }
...
router.get('/app-settings', auth_1.authenticate, (req, res) => {
    const authReq = req;
    ...
    for (const key of ADMIN_SETTINGS_KEYS) {
        ...
        if (row)
            result[key] = key === 'smtp_pass' ? '••••••••' : row.value;
    }
    res.json(result);
});
router.put('/app-settings', auth_1.authenticate, (req, res) => {
    ...
    if (user?.role !== 'admin')
        return res.status(403).json({ error: 'Admin access required' });
    const { allow_registration, allowed_file_types, require_mfa } = req.body;
    if (require_mfa === true || require_mfa === 'true') {
        const adminMfa = database_1.db.prepare('SELECT mfa_enabled FROM users WHERE id = ?').get(authReq.user.id);
...
router.post('/mfa/enable', auth_1.authenticate, (req, res) => {
    ...
    database_1.db.prepare('UPDATE users SET mfa_enabled = 1, mfa_secret = ?, mfa_backup_codes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(enc, JSON.stringify(backupHashes), authReq.user.id);
    mfaSetupPending.delete(authReq.user.id);
    (0, auditLog_1.writeAudit)({ userId: authReq.user.id, action: 'user.mfa_enable', ip: (0, auditLog_1.getClientIp)(req) });
...
router.post('/mfa/disable', auth_1.authenticate, rateLimiter(5, RATE_LIMIT_WINDOW), (req, res) => {
    ...
    database_1.db.prepare('UPDATE users SET mfa_enabled = 0, mfa_secret = NULL, mfa_backup_codes = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(authReq.user.id);
    mfaSetupPending.delete(authReq.user.id);
    (0, auditLog_1.writeAudit)({ userId: authReq.user.id, action: 'user.mfa_disable', ip: (0, auditLog_1.getClientIp)(req) });
    res.json({ success: true, mfa_enabled: false });
});
// --- MCP Token Management ---
router.get('/mcp-tokens', auth_1.authenticate, (req, res) => {
    const authReq = req;
    const tokens = database_1.db.prepare('SELECT id, name, token_prefix, created_at, last_used_at FROM mcp_tokens WHERE user_id = ? ORDER BY created_at DESC').all(authReq.user.id);
    res.json({ tokens });
});
router.post('/mcp-tokens', auth_1.authenticate, rateLimiter(5, RATE_LIMIT_WINDOW), (req, res) => {
    const authReq = req;
    const { name } = req.body;
    if (!name?.trim())
    ...
    const tokenCount = database_1.db.prepare('SELECT COUNT(*) as count FROM mcp_tokens WHERE user_id = ?').get(authReq.user.id).count;
    if (tokenCount >= 10)
        return res.status(400).json({ error: 'Maximum of 10 tokens per user reached' });
    const rawToken = 'trek_' + (0, crypto_2.randomBytes)(24).toString('hex');
    ...
    const result = database_1.db.prepare('INSERT INTO mcp_tokens (user_id, name, token_hash, token_prefix) VALUES (?, ?, ?, ?)').run(authReq.user.id, name.trim(), tokenHash, tokenPrefix);
...
router.delete('/mcp-tokens/:id', auth_1.authenticate, (req, res) => {
    const authReq = req;
    const { id } = req.params;
    const token = database_1.db.prepare('SELECT id FROM mcp_tokens WHERE id = ? AND user_id = ?').get(id, authReq.user.id);
    if (!token)
        return res.status(404).json({ error: 'Token not found' });
    database_1.db.prepare('DELETE FROM mcp_tokens WHERE id = ?').run(id);
    (0, mcp_1.revokeUserSessions)(authReq.user.id);
    res.json({ success: true });
});
