...
const storage = multer_1.default.diskStorage({
    destination: (_req, _file, cb) => {
        if (!fs_1.default.existsSync(s3_2.tempDir))
            fs_1.default.mkdirSync(s3_2.tempDir, { recursive: true });
        cb(null, s3_2.tempDir);
    },
...
// Upload file
router.post('/', auth_1.authenticate, tripAccess_1.requireTripAccess, auth_1.demoUploadBlock, upload.single('file'), (0, s3Upload_1.s3Upload)('files'), (req, res) => {
    const authReq = req;
    const { tripId } = req.params;
...
router.delete('/:id/permanent', auth_1.authenticate, async (req, res) => {
    ...
    if (!file)
        return res.status(404).json({ error: 'File not found in trash' });
    await (0, s3_1.deleteFile)(`files/${file.filename}`);
    database_1.db.prepare('DELETE FROM trip_files WHERE id = ?').run(id);
    res.json({ success: true });
    (0, websocket_1.broadcast)(tripId, 'file:deleted', { fileId: Number(id) }, req.headers['x-socket-id']);
});
// Empty entire trash
router.delete('/trash/empty', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const { tripId } = req.params;
    const trip = verifyTripOwnership(tripId, authReq.user.id);
    if (!trip)
