...
const gpxUpload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 } });
...
router.post('/import/gpx', auth_1.authenticate, tripAccess_1.requireTripAccess, gpxUpload.single('file'), (req, res) => {
    const { tripId } = req.params;
    const file = req.file;
    if (!file)
        return res.status(400).json({ error: 'No file uploaded' });
    const xml = file.buffer.toString('utf-8');
    const parseCoords = (attrs) => {
    ...
    const extractName = (body) => { const m = body.match(/<name[^>]*>([\s\S]*?)<\/name>/i); return m ? stripCdata(m[1]) : null; };
    const extractDesc = (body) => { const m = body.match(/<desc[^>]*>([\s\S]*?)<\/desc>/i); return m ? stripCdata(m[1]) : null; };
    const waypoints = [];
