...
router.post('/bucket-list', (req, res) => {
    ...
    const result = database_1.db.prepare('INSERT INTO bucket_list (user_id, name, lat, lng, country_code, notes, target_date) VALUES (?, ?, ?, ?, ?, ?, ?)').run(authReq.user.id, name.trim(), lat ?? null, lng ?? null, country_code ?? null, notes ?? null, target_date ?? null);
...
router.put('/bucket-list/:id', (req, res) => {
    ...
    database_1.db.prepare(`UPDATE bucket_list SET
    name = COALESCE(?, name),
    ...
    WHERE id = ?`).run(name?.trim() || null, notes !== undefined ? 1 : 0, notes !== undefined ? (notes || null) : null, lat !== undefined ? 1 : 0, lat !== undefined ? (lat || null) : null, lng !== undefined ? 1 : 0, lng !== undefined ? (lng || null) : null, country_code !== undefined ? 1 : 0, country_code !== undefined ? (country_code || null) : null, target_date !== undefined ? 1 : 0, target_date !== undefined ? (target_date || null) : null, req.params.id);
