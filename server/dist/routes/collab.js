...
router.delete('/notes/:id', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    const { tripId, id } = req.params;
    if (!verifyTripAccess(tripId, authReq.user.id))
        return res.status(404).json({ error: 'Trip not found' });
    ...
    for (const f of noteFiles) {
        await (0, s3_1.deleteFile)(f.filename);
    }
...
router.post('/notes/:id/files', auth_1.authenticate, noteUpload.single('file'), (0, s3Upload_1.s3Upload)('files'), (req, res) => {
    const authReq = req;
    const { tripId, id } = req.params;
...
router.delete('/notes/:id/files/:fileId', auth_1.authenticate, async (req, res) => {
    const authReq = req;
    ...
    await (0, s3_1.deleteFile)(file.filename);
...
router.post('/messages', auth_1.authenticate, (0, validate_1.validateStringLengths)({ text: 5000 }), (req, res) => {
  ...
  `).get(result.lastInsertRowid);
    ...
    Promise.resolve().then(() => __importStar(require('../services/notifications'))).then(({ notifyTripMembers }) => {
