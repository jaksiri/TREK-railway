...
router.post('/resolve-url', auth_1.authenticate, async (req, res) => {
    const { url } = req.body;
    ...
    try {
        ...
        if (url.includes('goo.gl') || url.includes('maps.app')) {
            const redirectRes = await (0, node_fetch_1.default)(url, { redirect: 'follow', signal: AbortSignal.timeout(10000) });
        ...
        const nominatimRes = await (0, node_fetch_1.default)(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&addressdetails=1`, { headers: { 'User-Agent': 'TREK-Travel-Planner/1.0' }, signal: AbortSignal.timeout(8000) });
